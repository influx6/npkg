...
// and 3/ it would not remain sortable. To validate a base32 `nxid`, expect a 20 chars long,
// all lowercase sequence of `a` to `v` letters and `0` to `9` numbers (`[0-9a-v]{20}`).
//
// UUID is 16 bytes (128 bits), snowflake is 8 bytes (64 bits), nxid stands in between
...
func readMachineID() []byte {
	...
	if err == nil && len(hid) != 0 {
		...
		if _, randErr := rand.Reader.Read(id); randErr != nil {
			panic(fmt.Errorf("nxid: cannot get hostname nor generate a random number: %v; %v", err, randErr))
		}
	}
	return id
}
...
func randInt() uint32 {
	...
	if _, err := rand.Reader.Read(b); err != nil {
		panic(fmt.Errorf("nxid: cannot generate random number: %v;", err))
	}
...
func (id *ID) Scan(value interface{}) (err error) {
	...
	case nil:
		*id = nilID
		return nil
	default:
		return fmt.Errorf("nxid: scanning unsupported type: %T", value)
	}
}
...
// NilID returns a zero value for `nxid.ID`.
func NilID() ID {
	return nilID
}

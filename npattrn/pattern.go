...
// Priority returns the priority status of this giving npattrn.
func (m *matchProvider) Priority() int {
	return m.priority
}

// Pattern returns the npattrn string for this matcher.
func (m *matchProvider) Pattern() string {
	return m.pattern
}

// Validate returns true/false if the giving string matches the npattrn, returning
...
// interface, with each made of each segment of the npattrn.
...
// Segment returns a Matchable for a specific part of a npattrn eg. :name, age,
// {id:[\\d+]}.

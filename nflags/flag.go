// Package nflags creates a augmentation on the native nflags package.
package nflags
...
const (
	usageTml = `Usage: {{ toLower .Title}} [nflags] [command]
...
⡿ OTHERS:
	Run '{{toLower .Title}} nflags' to print all nflags of all commands.

⡿ WARNING:
	Uses internal flag package so nflags must precede command name.
	e.g '{{toLower .Title}} -cmd.flag=4 run'
`

	cmdUsageTml = `Command: {{toLower .Title}} [nflags] {{ toLower .Cmd.Name}}
...
// DurationFlag implements a structure for parsing duration nflags.
...
// Float64Flag implements a structure for parsing float64 nflags.
...
// UInt64Flag implements a structure for parsing uint64 nflags.
...
// Int64Flag implements a structure for parsing int64 nflags.
...
// UIntFlag implements a structure for parsing uint nflags.
...
// IntFlag implements a structure for parsing int nflags.
type IntFlag struct {
	Name       string
	Desc       string
	Default    int
...
// BoolFlag implements a structure for parsing bool nflags.
...
// TBoolFlag implements a structure for parsing bool nflags that are true by default.
...
// StringFlag implements a structure for parsing string nflags.
...
type Command struct {
	...
	// AllowDefault is used when only one command is provided to nflags, and we want it
...
// Run adds all commands and appropriate nflags for each commands.
...
func Run(title string, cmds ...Command) {
	...
	if tml, err := template.New("nflags.Usage").Funcs(defs).Parse(usageTml); err == nil {
		var bu bytes.Buffer
